    ...
    private void runRequests(int requestCount, String ipAddr, boolean countRequests) {
        for (int i = 0; i < requestCount; i++) {
            ...
            try {
                ...
                if (countRequests && albumID != null) {
                    counter.incrementSuccessfulRequests(); // Increment counter for the successful POST
                }
            } catch (Exception e) {
                ...
                continue; // Skip to the next iteration if the POST request failed
            }
            //System.out.println("######## -> " + albumID);
            // If the POST was successful, we proceed to send likes/dislikes
            try {
                // Post two likes
                ReviewLogic.postReview(ipAddr, albumID, "like", counter);
                if (countRequests) {
                    counter.incrementSuccessfulRequests(); // Increment counter for the first like
                }
            } catch (Exception e) {
                logger.error("Error posting like", e);
                if (countRequests) {
                    counter.incrementFailedRequests();
                }
            }
            ...
            try {
                ...
                ReviewLogic.postReview(ipAddr, albumID, "like", counter);
                if (countRequests) {
                    counter.incrementSuccessfulRequests(); // Increment counter for the second like
                }
            } catch (Exception e) {
                logger.error("Error posting like", e);
                if (countRequests) {
            ...
            try {
                // Post one dislike
                ReviewLogic.postReview(ipAddr, albumID, "dislike", counter);
                if (countRequests) {
                    counter.incrementSuccessfulRequests(); // Increment counter for the dislike
                }
            } catch (Exception e) {
                logger.error("Error posting dislike", e);
                if (countRequests) {
                    counter.incrementFailedRequests();
                }
            }
